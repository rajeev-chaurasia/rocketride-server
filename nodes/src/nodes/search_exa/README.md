...
Before serializing results, the node validates every `url`, `image`, and `favicon` field. Each must use `http` or `https` and resolve to a public IP address. A result with an invalid primary `url` is removed; invalid images and favicons are removed while retaining their result. This sanitization is skipped when `ROCKETRIDE_MOCK` is set.
